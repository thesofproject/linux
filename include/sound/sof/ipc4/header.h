 ...
 * _reserved_0 : 1;
 *
 * where bits 24 .. 31 are used globally, others (rsvd0) will
 * have different definitions for each message type.
 */

/* Value of msg_tgt field - must fit into 1 bit */
...
enum sof_ipc4_global_msg {
	...
	/* GAP HERE- DO NOT USE 28 .. 30 */

	SOF_IPC4_GLB_MAX_IXC_MESSAGE_TYPE = 31
};

/* Value of rsp field - must fit into 1 bit */
...
enum sof_ipc4_sample_type {
	...
	SOF_IPC4_SIGNED_INTEGER, /* signed integer */
	SOF_IPC4_UNSIGNED_INTEGER, /* unsigned integer */
	SOF_IPC4_FLOAT, /* unsigned integer */
};

struct sof_ipc4_audio_format {
	uint32_t sampling_frequency; /* sof_ipc4_sampling_frequency */
	uint32_t bit_depth; /* sof_ipc4_bit_depth */
	uint32_t ch_map;
...
/*
 * The bit allocation of a notification message is
 * uint32_t rsvd0      : 16;
...
/* Value of notif_type field - must fit into 8 bits */
