				...
				u64 vmid)
{
	...
	if (FIELD_GET(FFA_NOTIF_RECEIVER_ENDP_MASK, endp_id) != vmid) {
		ffa_to_smccc_res(res, FFA_RET_INVALID_PARAMETERS);
		return;
	}
			     ...
			     u64 vmid)
{
	...
	if (FIELD_GET(FFA_NOTIF_SENDER_ENDP_MASK, endp_id) != vmid) {
		ffa_to_smccc_res(res, FFA_RET_INVALID_PARAMETERS);
		return;
	}
...
bool kvm_guest_ffa_handler(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	...
	case FFA_NOTIFICATION_UNBIND:
		...
		goto out_guest;
	case FFA_NOTIFICATION_SET:
		do_ffa_notif_set(&res, ctxt, hyp_vcpu_to_ffa_handle(hyp_vcpu));
		goto out_guest;
	case FFA_NOTIFICATION_GET:
		do_ffa_notif_get(&res, ctxt, hyp_vcpu_to_ffa_handle(hyp_vcpu));
		goto out_guest;
	case FFA_NOTIFICATION_INFO_GET:
	case FFA_FN64_NOTIFICATION_INFO_GET:
