...
static void tty_rpmsg_copy(void *dst, const void *src, size_t len)
{
...
static int tty_rpmsg_do_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	struct rpmsg_device *rpdev = dev_get_drvdata(tty->dev);
	...
	msg->count = count;
	tty_rpmsg_copy(msg->data, buf, count);
...
static int tty_rpmsg_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	struct tty_port *p = tty->port;
	struct tty_rpmsg_port *port = to_tty_rpmsg_port(p);
	int space, written;
...
{
	struct tty_rpmsg_port *port = container_of(work, struct tty_rpmsg_port, work);
...
static int rpmsg_tty_probe(struct rpmsg_device *rpdev)
{
	...
	if (of_property_read_u32(rpdev->dev.of_node, "max-size", &max_size) < 0) {
		/*
		 * don't limit receive buffer size by default since:
		 * 1.tty framework don't notify us when the user space read the data from buffer
		 * 2.so it's difficult to send RPMSG_TTY_WAKEUP back once some space is available
		 */
		max_size = INT_MAX;
	}
	...
	port->driver->type         = TTY_DRIVER_TYPE_SERIAL | TTY_DRIVER_REAL_RAW;
	...
	dev = tty_port_register_device_attr(&port->port,
		port->driver, 0, &rpdev->dev, rpdev, NULL);
	if (IS_ERR(dev)) {
		ret = PTR_ERR(dev);
		goto tty_unregister;
	}
			      ...
			      void *data, int len, void *priv, u32 src)
{
	...
	} else if (cmd < ARRAY_SIZE(rpmsg_tty_handler) && rpmsg_tty_handler[cmd])
...
module_driver(rpmsg_tty_driver,
