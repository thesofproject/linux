...
/**
 * cdns_update_peripheral_status_work - update peripheral status in a work
 * since we will need to handle other interrupts eg. CDNS_MCP_INT_RX_WL
 * during the update peripheral process.
...
update_status:
	...
	device0_status = cdns_readl(cdns, CDNS_MCP_PERIPHERAL_STAT);
	device0_status &= 3;

	if (device0_status == SDW_PERIPHERAL_ATTACHED) {
		if (retry_count++ < SDW_MAX_DEVICES) {
			dev_dbg_ratelimited(cdns->dev,
					    "Device0 detected after clearing status, iteration %d\n",
					    retry_count);
			peripheral_intstat = CDNS_MCP_PERIPHERAL_INTSTAT_ATTACHED;
			goto update_status;
		} else {
			dev_err_ratelimited(cdns->dev,
