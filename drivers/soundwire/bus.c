...
static int sdw_delete_peripheral(struct device *dev, void *data)
{
	...
	if (peripheral->dev_num) { /* clear dev_num if assigned */
		...
		if (bus->dev_num_ida_min)
			ida_free(&sdw_peripheral_ida, peripheral->dev_num);
	}
...
static int
sdw_nwrite_no_pm(struct sdw_peripheral *peripheral, u32 addr, size_t count, const u8 *val)
{
	struct sdw_msg msg;
	int ret;

	ret = sdw_fill_msg(&msg, peripheral, addr, count,
			   peripheral->dev_num, SDW_MSG_FLAG_WRITE, (u8 *)val);
	if (ret < 0)
		return ret;
...
int sdw_nread(struct sdw_peripheral *peripheral, u32 addr, size_t count, u8 *val)
{
	...
	ret = pm_runtime_resume_and_get(&peripheral->dev);
	if (ret < 0 && ret != -EACCES)
		return ret;
 ...
 */
int sdw_nwrite(struct sdw_peripheral *peripheral, u32 addr, size_t count, const u8 *val)
{
	int ret;
...
static int sdw_get_device_num(struct sdw_peripheral *peripheral)
{
	...
	if (peripheral->bus->dev_num_ida_min) {
		bit = ida_alloc_range(&sdw_peripheral_ida,
				      peripheral->bus->dev_num_ida_min, SDW_MAX_DEVICES,
				      GFP_KERNEL);
		if (bit < 0)
			goto err;
	} else {
					 ...
					 enum sdw_peripheral_status status)
{
	...
	dev_vdbg(bus->dev,
		 "changing status peripheral %d status %d new status %d\n",
		 peripheral->dev_num, peripheral->status, status);

	if (status == SDW_PERIPHERAL_UNATTACHED) {
		dev_dbg(&peripheral->dev,
			"initializing enumeration and init completion for Peripheral %d\n",
			peripheral->dev_num);
		...
		dev_dbg(&peripheral->dev,
		   ...
		   (peripheral->status == SDW_PERIPHERAL_UNATTACHED)) {
		dev_dbg(&peripheral->dev,
			"signaling enumeration completion for Peripheral %d\n",
			peripheral->dev_num);
					    ...
					    enum sdw_clk_stop_type type)
{
	...
	if (peripheral->probed)  {
		...
		if (drv->ops && drv->ops->clk_stop)
			ret = drv->ops->clk_stop(peripheral, mode, type);
	}
		...
		if (peripheral_notify) {
			...
			if (peripheral->probed) {
				...
				if (drv->ops && drv->ops->interrupt_callback) {
					...
					drv->ops->interrupt_callback(peripheral, &peripheral_intr);
				}
			}
					...
					enum sdw_peripheral_status status)
{
	...
	if (peripheral->probed) {
		...
		if (drv->ops && drv->ops->update_status)
			ret = drv->ops->update_status(peripheral, status);
	}
				 ...
				 enum sdw_peripheral_status status[])
{
	...
	for (i = 1; i <= SDW_MAX_DEVICES; i++) {
		...
		if (status[i] == SDW_PERIPHERAL_UNATTACHED &&
		    peripheral->status != SDW_PERIPHERAL_UNATTACHED) {
			dev_warn(&peripheral->dev, "Peripheral %d state check1: UNATTACHED, status was %d\n",
				 i, peripheral->status);
	...
	for (i = 1; i <= SDW_MAX_DEVICES; i++) {
		...
		case SDW_PERIPHERAL_UNATTACHED:
			...
			dev_warn(&peripheral->dev, "Peripheral %d state check2: UNATTACHED, status was %d\n",
				 i, peripheral->status);
		...
		if (attached_initializing) {
			dev_dbg(&peripheral->dev,
				"signaling initialization completion for Peripheral %d\n",
				peripheral->dev_num);
			 ...
			 * suspended. If the 'update_peripheral_status' callback initiates
			 ...
			 */
			pm_request_resume(&peripheral->dev);
		}
	}
