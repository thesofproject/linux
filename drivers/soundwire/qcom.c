...
#define SWRM_MCP_PERIPHERAL_STATUS				0x1090
#define SWRM_MCP_PERIPHERAL_STATUS_MASK				GENMASK(1, 0)
#define SWRM_MCP_PERIPHERAL_STATUS_SZ				2
...
static int qcom_swrm_get_alert_peripheral_dev_num(struct qcom_swrm_ctrl *ctrl)
{
	...
	ctrl->reg_read(ctrl, SWRM_MCP_PERIPHERAL_STATUS, &val);

	for (dev_num = 0; dev_num < SDW_MAX_DEVICES; dev_num++) {
		status = (val >> (dev_num * SWRM_MCP_PERIPHERAL_STATUS_SZ));

		if ((status & SWRM_MCP_PERIPHERAL_STATUS_MASK) == SDW_PERIPHERAL_ALERT) {
...
static void qcom_swrm_get_device_status(struct qcom_swrm_ctrl *ctrl)
{
	...
	for (i = 0; i < SDW_MAX_DEVICES; i++) {
		...
		s = (val >> (i * 2));
		s &= SWRM_MCP_PERIPHERAL_STATUS_MASK;
		ctrl->status[i] = s;
	}
}
					     ...
					     int devnum)
{
	...
	ctrl->reg_read(ctrl, SWRM_MCP_PERIPHERAL_STATUS, &status);
	status = (status >> (devnum * SWRM_MCP_PERIPHERAL_STATUS_SZ));
	status &= SWRM_MCP_PERIPHERAL_STATUS_MASK;
		...
		for (i = 0; i < SWRM_INTERRUPT_MAX; i++) {
			...
			case SWRM_INTERRUPT_STATUS_CHANGE_ENUM_PERIPHERAL_STATUS:
				dev_err_ratelimited(swrm->dev, "%s: SWR new peripheral attached\n",
						    __func__);
				swrm->reg_read(swrm, SWRM_MCP_PERIPHERAL_STATUS, &peripheral_status);
...
static int __maybe_unused swrm_runtime_resume(struct device *dev)
{
	...
	if (ctrl->clock_stop_not_supported) {
		...
		sdw_handle_peripheral_status(&ctrl->bus, ctrl->status);
	} else {
		reset_control_reset(ctrl->audio_cgcr);
