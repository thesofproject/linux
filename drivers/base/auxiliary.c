				...
				const char *modname)
{
	...
	/*
	 * The driver name will be <module_name>.<name>, so we need
	 * to add "2" bytes, one for "." and another for the NULL-termination.
	 */
	name = kmalloc(mlen + dlen + 1, GFP_KERNEL);
	if (!name)
		return -ENOMEM;
	...
	if (dlen) {
		strcat(name, ".");
		strcat(name, auxdrv->name);
	}

	auxdrv->driver.owner = owner;
	auxdrv->driver.bus = &auxiliary_bus_type;
	auxdrv->driver.mod_name = modname;
	auxdrv->driver.name = name;
