	...
	while (remaining >= sizeof(struct mbox_tlv)) {
		...
		case BTINTEL_PCIE_TLV_TYPE_SDS_MEM_ADDRESS:
			if (tlv_len == 16 && hw_variant > BTINTEL_HWID_BZRI) {
				data->dump_info.sds_start_addr_start = get_unaligned_le32(&tlv->val[0]);
				data->dump_info.sds_start_addr_end = get_unaligned_le32(&tlv->val[4]);
				data->dump_info.sds_iosf_data_addr_start = get_unaligned_le32(&tlv->val[8]);
				data->dump_info.sds_iosf_data_addr_end = get_unaligned_le32(&tlv->val[12]);
			} else if (tlv_len == 24 && (hw_variant == BTINTEL_HWID_BZRI ||
						     hw_variant == BTINTEL_HWID_BZRIW)) {
				data->dump_info.sds_fixed_rom_addr_start = get_unaligned_le32(&tlv->val[0]);
				data->dump_info.sds_fixed_rom_addr_end = get_unaligned_le32(&tlv->val[4]);
				data->dump_info.sds_start_addr_start = get_unaligned_le32(&tlv->val[8]);
				data->dump_info.sds_start_addr_end = get_unaligned_le32(&tlv->val[12]);
				data->dump_info.sds_iosf_data_addr_start = get_unaligned_le32(&tlv->val[16]);
				data->dump_info.sds_iosf_data_addr_end = get_unaligned_le32(&tlv->val[20]);
			} else {
