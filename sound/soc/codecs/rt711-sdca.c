...
//
// rt711-sdca.c -- rt711 ALSA SoC audio driver
//
...
static int rt711_sdca_index_write(struct rt711_priv *rt711,
		unsigned int nid, unsigned int reg, unsigned int value)
{
	int ret;
	struct regmap *regmap = rt711->regmap;
	unsigned int addr = addr = ((nid << 20) | reg);
...
static int rt711_sdca_index_read(struct rt711_priv *rt711,
		unsigned int nid, unsigned int reg, unsigned int *value)
{
	int ret;
	struct regmap *regmap = rt711->regmap;
...
static int rt711_sdca_index_update_bits(struct rt711_priv *rt711,
	unsigned int nid, unsigned int reg, unsigned int mask, unsigned int val)
{
	unsigned int tmp, orig;
	int ret;

	ret = rt711_sdca_index_read(rt711, nid, reg, &orig);
	if (ret < 0)
		return ret;
...
static void rt711_sdca_reset(struct rt711_priv *rt711)
{
...
static int rt711_sdca_calibration(struct rt711_priv *rt711)
{
	...
	while (!(val & 0x40)) {
		if (loop >= 500) {
			pr_err("%s, RC calibration time-out!\n",
							__func__);
			ret = -ETIMEDOUT;
			break;
		}
	...
	rt711_sdca_index_update_bits(rt711, RT711_VENDOR_REG, RT711_FSM_CTL,
		0xf, RT711_CALI_CTL);
...
static unsigned int rt711_sdca_button_detect(struct rt711_priv *rt711)
{
	...
	ret = regmap_read(rt711->regmap,
		RT711_SDCA_CTL(FUN_HID, ENT_HID01,
		CTL_HIDTX_CURRENT_OWNER, 0), &owner);
	if (ret < 0)
		goto _end_btn_det_;

	/* if owner is device that there is no button event from device */
	if (owner == 1)
		goto _end_btn_det_;
	...
	ret = regmap_read(rt711->regmap,
		RT711_SDCA_CTL(FUN_HID, ENT_HID01,
		CTL_HIDTX_MESSAGE_OFFSET, 0), &offset);
	if (ret < 0)
		goto _end_btn_det_;
	...
	if (owner == 0) {
		...
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_HID, ENT_HID01,
			CTL_HIDTX_SET_OWNER_TO_DEVICE, 0), 0x01);
	}
...
static int rt711_sdca_headset_detect(struct rt711_priv *rt711)
{
	...
	/* get detected_mode */
	ret = regmap_read(rt711->regmap,
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_GE49,
		CTL_DETECTED_MODE, 0), &det_mode);
	if (ret < 0)
		goto io_error;
...
static void rt711_sdca_jack_detect_handler(struct work_struct *work)
{
	...
	int btn_type = 0, ret;
	unsigned int jack_status = 0, check_bit;
	...
	ret = regmap_read(rt711->regmap,
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_GE49,
		CTL_DETECTED_MODE, 0), &jack_status);
	if (ret < 0)
		goto io_error;
	check_bit = BIT(0);

	/* pin attached */
	if (jack_status & check_bit) {
		/* jack in */
...
static void rt711_sdca_jack_init(struct rt711_priv *rt711)
{
	...
	if (rt711->hs_jack) {
		...
		rt711_sdca_index_update_bits(rt711, RT711_VENDOR_HDA_CTL,
			RT711_PUSH_BTN_INT_CTL6,
			0x80f0, 0x80f0);
		rt711_sdca_index_update_bits(rt711, RT711_VENDOR_HDA_CTL,
			RT711_PUSH_BTN_INT_CTL2,
			0x11dd, 0x11dd);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mc->shift == 8) /* boost gain */
		...
		if (adc_vol_flag && gain_l_val > mc->shift)
			...
			gain_l_val = mc->max - gain_l_val;
			gain_l_val -= (mc->max - mc->shift);
			gain_l_val *= 75;
		}
		gain_l_val <<= 8;
		gain_l_val /= 100;
	...
	if (mc->shift == 8) /* boost gain */
		...
		if (adc_vol_flag && gain_r_val > mc->shift)
			...
			gain_r_val = mc->max - gain_r_val;
			gain_r_val -= (mc->max - mc->shift);
			gain_r_val *= 75;
		}
		gain_r_val <<= 8;
		gain_r_val /= 100;
...
static const struct snd_kcontrol_new rt711_sdca_snd_controls[] = {
	SOC_DOUBLE_R_EXT_TLV("FU05 Playback Volume",
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_USER_FU05,
		CTL_FU_VOLUME, CH_L),
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_USER_FU05,
		CTL_FU_VOLUME, CH_R), 0x57, 0x57, 0,
		rt711_sdca_set_gain_get, rt711_sdca_set_gain_put, out_vol_tlv),
	SOC_DOUBLE_R("FU1E Capture Switch",
		RT711_SDCA_CTL(FUN_MIC_ARRAY, ENT_USER_FU1E,
		CTL_FU_MUTE, CH_L),
		RT711_SDCA_CTL(FUN_MIC_ARRAY, ENT_USER_FU1E,
		CTL_FU_MUTE, CH_R), 0, 1, 1),
	SOC_DOUBLE_R("FU0F Capture Switch",
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_USER_FU0F,
		CTL_FU_MUTE, CH_L),
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_USER_FU0F,
		CTL_FU_MUTE, CH_R), 0, 1, 1),
	SOC_DOUBLE_R_EXT_TLV("FU1E Capture Volume",
		...
		CTL_FU_VOLUME, CH_L),
		RT711_SDCA_CTL(FUN_MIC_ARRAY, ENT_USER_FU1E,
		CTL_FU_VOLUME, CH_R), 0x17, 0x3f, 0,
	...
	SOC_DOUBLE_R_EXT_TLV("FU44 Gain Volume",
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_PLATFORM_FU44,
		CTL_FU_CH_GAIN, CH_L),
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_PLATFORM_FU44,
		CTL_FU_CH_GAIN, CH_R), 8, 3, 0,
		rt711_sdca_set_gain_get, rt711_sdca_set_gain_put, mic_vol_tlv),
	SOC_DOUBLE_R_EXT_TLV("FU15 Gain Volume",
		RT711_SDCA_CTL(FUN_MIC_ARRAY, ENT_PLATFORM_FU15,
		CTL_FU_CH_GAIN, CH_L),
		RT711_SDCA_CTL(FUN_MIC_ARRAY, ENT_PLATFORM_FU15,
		CTL_FU_CH_GAIN, CH_R), 8, 3, 0,
		rt711_sdca_set_gain_get, rt711_sdca_set_gain_put, mic_vol_tlv),
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	val2 = (val2 & (0x7 << mask_sft)) >> mask_sft;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_USER_FU05,
			CTL_FU_MUTE, CH_L), UNMUTE);
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_USER_FU05,
			CTL_FU_MUTE, CH_R), UNMUTE);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_USER_FU05,
			CTL_FU_MUTE, CH_L), MUTE);
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_USER_FU05,
			CTL_FU_MUTE, CH_R), MUTE);
		break;
	}
	return 0;
}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_PDE28,
			CTL_REQ_POWER_STATE, 0), PS0);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_PDE28,
			CTL_REQ_POWER_STATE, 0), PS3);
		break;
	}
	return 0;
}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_PDE29,
			CTL_REQ_POWER_STATE, 0), PS0);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		regmap_write(rt711->regmap,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_MIC_ARRAY, ENT_PDE2A,
			CTL_REQ_POWER_STATE, 0), PS0);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		regmap_write(rt711->regmap,
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		regmap_read(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_GE49,
			CTL_SELECTED_MODE, 0), &sel_mode);
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_LINE1,
			CTL_VENDOR_DEF, 0), 0x1);
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_GE49,
			CTL_SELECTED_MODE, 0), 0x7);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_LINE1,
			CTL_VENDOR_DEF, 0), 0x0);
		if (sel_mode != 0xffff)
			regmap_write(rt711->regmap,
				RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_GE49,
				CTL_SELECTED_MODE, 0), sel_mode);
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_PDELINE2,
			CTL_REQ_POWER_STATE, 0), PS0);
		regmap_write(rt711->regmap,
			RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_LINE2,
			CTL_VENDOR_DEF, 0), 0x1);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		regmap_write(rt711->regmap,
				...
				struct snd_soc_dai *dai)
{
	...
	/* 48Khz */
	regmap_write(rt711->regmap,
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_CS01,
		CTL_SAMPLE_FREQ_INDEX, 0), 0x09);
	regmap_write(rt711->regmap,
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_CS11,
		CTL_SAMPLE_FREQ_INDEX, 0), 0x09);
	regmap_write(rt711->regmap,
		RT711_SDCA_CTL(FUN_MIC_ARRAY, ENT_CS1F,
		CTL_SAMPLE_FREQ_INDEX, 0), 0x09);
...
int rt711_sdca_io_init(struct device *dev, struct sdw_slave *slave)
{
	...
	rt711_sdca_index_update_bits(rt711, RT711_VENDOR_ANALOG_CTL,
		RT711_MISC_POWER_CTL4,
		0x6000, 0x6000);

	rt711_sdca_index_update_bits(rt711, RT711_VENDOR_REG,
		RT711_COMBO_JACK_AUTO_CTL3,
		0x000c, 0x000c);
	...
	/* HP output enable */
	regmap_write(rt711->regmap,
		RT711_SDCA_CTL(FUN_JACK_CODEC, ENT_OT1,
		CTL_VENDOR_DEF, 0), 0x4);
