...
static void rt1308_apply_calib_params(struct rt1308_sdw_priv *rt1308)
{
	...
	dev_dbg(&rt1308->sdw_peripheral->dev, "%s m_btl_l=0x%x, m_btl_r=0x%x\n", __func__,
		efuse_m_btl_l, efuse_m_btl_r);
	dev_dbg(&rt1308->sdw_peripheral->dev, "%s c_btl_l=0x%x, c_btl_r=0x%x\n", __func__,
...
static int rt1308_sdw_remove(struct sdw_peripheral *peripheral)
{
	struct rt1308_sdw_priv *rt1308 = dev_get_drvdata(&peripheral->dev);
