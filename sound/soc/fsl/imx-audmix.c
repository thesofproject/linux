				   ...
				   struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_fmt(asoc_cpu_dai(rtd, 0), fmt);
	if (ret) {
		dev_err(dev, "failed to set cpu dai fmt: %d\n", ret);
		return ret;
	}

	ret = snd_soc_dai_set_sysclk(asoc_cpu_dai(rtd, 0), FSL_SAI_CLK_MAST1, 0, dir);
	if (ret) {
	 ...
	 */
	ret = snd_soc_dai_set_tdm_slot(asoc_cpu_dai(rtd, 0), BIT(channels) - 1,
				       BIT(channels) - 1, 8, 32);
	if (ret)
