			       ...
			       int pin_id)
{
	...
	if (swidget->init_blob_style == SOF_INIT_BLOB_BASE_WITH_MOD_EXT) {
		...
		if (pin_type == SOF_PIN_TYPE_SOURCE && swidget->payload_with_output_fmt) {
			/* Output format follows base module config */
			const struct sof_ipc4_audio_format *out_fmt = (void *)base_config +
				sizeof(*base_config);
			return out_fmt;
		}
	...
	/*
	 * Return the pin format from sof_ipc4_base_module_cfg_ext structure if module
	 * uses base module config + base module config extension init blob.
	 */
	...
	if (pin_type == SOF_PIN_TYPE_SINK) {
		pin_start = 0;
		pin_end = base_ext->num_sink_pin_fmts;
	} else {
		pin_start = base_ext->num_sink_pin_fmts;
		pin_end = base_ext->num_sink_pin_fmts + base_ext->num_source_pin_fmts;
	}
					   ...
					   struct snd_pcm_hw_params *pipeline_params, int dir)
{
	 ...
	 * module specific extension style init blob. it will be setup based on audio format
	 * tokens in topology,
	 */
	if (swidget->init_blob_style == SOF_INIT_BLOB_BASE_WITH_MOD_EXT &&
	    swidget->payload_with_output_fmt)
	...
	/* Make sure we have enough data for base module extension structure */
	if (swidget->init_blob_style == SOF_INIT_BLOB_BASE_WITH_BASE_EXT) {
		...
		size = sizeof(struct sof_ipc4_base_module_cfg_ext) +
					base_ext->priv_param_len +
					(base_ext->num_sink_pin_fmts +
					base_ext->num_source_pin_fmts) *
					   ...
					   int sink_id)
{
	...
	sink_format = sof_ipc4_get_widget_pin_format(sink_widget, SOF_PIN_TYPE_SINK, sink_id);
	if (!sink_format) {
