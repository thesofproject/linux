						 ...
						 size_t count, loff_t *ppos)
{
	...
	if (doffset >= SOF_IPC4_DEBUG_SLOT_SIZE)
		return -EINVAL;

	exposed_size = SOF_IPC4_DEBUG_SLOT_SIZE - doffset;

	if (pos < 0)
		return -EINVAL;
	if (pos >= exposed_size || !count)
		return 0;
	if (count > exposed_size - pos)
		count = exposed_size - pos;

	offset = sof_ipc4_find_debug_slot_offset_by_type(sdev, type);
	if (!offset)
		return -EFAULT;

	buf = kzalloc(exposed_size, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;

	sof_mailbox_read(sdev, offset + doffset, buf, exposed_size);
					       ...
					       size_t data_offset, const char *name)
{
	...
	exposed_size = SOF_IPC4_DEBUG_SLOT_SIZE - data_offset;

	/*
	 * Reject partial descriptor 15 if this interface must not cross the
	 * end of page 0 into the next slot.
	 */
	if (slot_type == SOF_IPC4_DEBUG_SLOT_DEBUG_STREAM &&
	    data_offset == SOF_IPC4_DEBUG_PAGE0_SLOT_OFFSET)
		return;
