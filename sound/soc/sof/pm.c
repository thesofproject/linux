...
static int sof_restore_pipelines(struct snd_sof_dev *sdev)
{
	...
	list_for_each_entry_reverse(sroute, &sdev->route_list, list) {
		struct sof_ipc_pipe_comp_connect *connect = sroute->private;
	...
	list_for_each_entry(swidget, &sdev->widget_list, list) {
		...
		case snd_soc_dapm_scheduler:
			swidget->complete =
				sof_complete_pipeline(sdev, swidget);
			break;
		default:
			break;
		}
...
static int sof_send_ctx_ipc(struct snd_sof_dev *sdev, int cmd)
{
...
static void sof_suspend_streams(struct snd_sof_dev *sdev)
{
	...
	list_for_each_entry(spcm, &sdev->pcm_list, list) {
		...
		if (substream && substream->runtime) {
			...
			/*
			 * SNDRV_PCM_TRIGGER_RESUME is not called for streams
			 * that are paused during suspend.
			 * So mark them explicitly so hw_params is restored.
			 */
...
static int sof_resume(struct device *dev)
{
	...
	/* load the firmware */
	ret = snd_sof_load_firmware(sdev, sdev->pdata->fw);
	if (ret < 0) {
		dev_err(sdev->dev,
	...
	/* notify DSP to restore context */
	ret = sof_send_ctx_ipc(sdev, SOF_IPC_PM_CTX_RESTORE);
	if (ret < 0)
		dev_err(sdev->dev,
...
static int sof_suspend(struct device *dev)
{
	...
	/* notify DSP to save context */
	ret = sof_send_ctx_ipc(sdev, SOF_IPC_PM_CTX_SAVE);
	if (ret < 0) {
		dev_err(sdev->dev,
