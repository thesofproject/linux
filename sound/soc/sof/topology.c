			       ...
			       struct snd_sof_dai *dai)
{
	...
	size_t ipc_size = sizeof(*comp_dai) + sizeof(swidget->comp_ext);
	int ret;

	comp_dai = kzalloc(ipc_size, GFP_KERNEL);
	if (!comp_dai)
		return -ENOMEM;

	/* configure dai IPC message */
	comp_dai->comp.hdr.size = ipc_size;
	...
	memcpy((u8 *)comp_dai + sizeof(*comp_dai),
	       &swidget->comp_ext, sizeof(swidget->comp_ext));
	comp_dai->comp.ext_data_offset = sizeof(*comp_dai);
			       ...
			       struct sof_ipc_comp_reply *r)
{
	...
	size_t ipc_size = sizeof(*host) + sizeof(swidget->comp_ext);
	int ret;

	host = kzalloc(ipc_size, GFP_KERNEL);
	if (!host)
		return -ENOMEM;

	/* configure host comp IPC message */
	host->comp.hdr.size = ipc_size;
	...
	/* append extended data to the end of the component */
	memcpy((u8 *)host + sizeof(*host),
	       &swidget->comp_ext, sizeof(swidget->comp_ext));
	host->comp.ext_data_offset = sizeof(*host);
				 ...
				 struct sof_ipc_comp_reply *r)
{
	...
	size_t ipc_size = sizeof(*mixer) + sizeof(swidget->comp_ext);
	int ret;

	mixer = kzalloc(ipc_size, GFP_KERNEL);
	if (!mixer)
		return -ENOMEM;

	/* configure mixer IPC message */
	mixer->comp.hdr.size = ipc_size;
	...
	/* append extended data to the end of the component */
	memcpy((u8 *)mixer + sizeof(*mixer),
	       &swidget->comp_ext, sizeof(swidget->comp_ext));
	mixer->comp.ext_data_offset = sizeof(*mixer);
			       ...
			       struct sof_ipc_comp_reply *r)
{
	...
	size_t ipc_size = sizeof(*src) + sizeof(swidget->comp_ext);
	int ret;

	src = kzalloc(ipc_size, GFP_KERNEL);
	if (!src)
		return -ENOMEM;

	/* configure src IPC message */
	src->comp.hdr.size = ipc_size;
	...
	/* append extended data to the end of the component */
	memcpy((u8 *)src + sizeof(*src),
	       &swidget->comp_ext, sizeof(swidget->comp_ext));
	src->comp.ext_data_offset = sizeof(*src);
				  ...
				  struct sof_ipc_comp_reply *r)
{
	...
	size_t ipc_size = sizeof(*tone) + sizeof(swidget->comp_ext);
	int ret;

	tone = kzalloc(ipc_size, GFP_KERNEL);
	if (!tone)
		return -ENOMEM;

	/* configure siggen IPC message */
	tone->comp.hdr.size = ipc_size;
	...
	/* append extended data to the end of the component */
	memcpy((u8 *)tone + sizeof(*tone),
	       &swidget->comp_ext, sizeof(swidget->comp_ext));
	tone->comp.ext_data_offset = sizeof(*tone);
