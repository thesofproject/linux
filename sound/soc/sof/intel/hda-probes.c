			   ...
			   struct snd_soc_dai *dai)
{
	...
	he_stream = hda_dsp_stream_get(sdev, cstream->direction, 0);
	if (!he_stream)
		return -EBUSY;

	hdac_stream(he_stream)->curr_pos = 0;
	hdac_stream(he_stream)->cstream = cstream;
	cstream->runtime->private_data = he_stream;

	return hdac_stream(he_stream)->stream_tag;
}
...
int hda_probe_compr_free(struct snd_sof_dev *sdev,
			 ...
			 struct snd_soc_dai *dai)
{
	struct hdac_ext_stream *he_stream = hda_compr_get_stream(cstream);
	int ret;

	ret = hda_dsp_stream_put(sdev, cstream->direction,
				 hdac_stream(he_stream)->stream_tag);
	if (ret < 0) {
		dev_dbg(sdev->dev, "stream put failed: %d\n", ret);
		return ret;
	}

	hdac_stream(he_stream)->cstream = NULL;
...
{
	struct hdac_ext_stream *he_stream = hda_compr_get_stream(cstream);
	struct hdac_stream *hstream = hdac_stream(he_stream);
			    ...
			    struct snd_soc_dai *dai)
{
	...
	tstamp->copied_total = hdac_stream(he_stream)->curr_pos;
