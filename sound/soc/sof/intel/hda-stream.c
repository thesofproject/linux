			       ...
			       int enable, u32 size)
{
	...
	/* set the SPIB value */
	sof_io_write(sdev, he_stream->spib_addr, size);
...
hda_dsp_stream_get(struct snd_sof_dev *sdev, int direction, u32 flags)
{
	...
	list_for_each_entry(s, &bus->stream_list, list) {
		if (s->direction == direction && !s->opened) {
			...
			hda_stream = container_of(he_stream,
...
int hda_dsp_stream_put(struct snd_sof_dev *sdev, int direction, int stream_tag)
{
	...
	list_for_each_entry(s, &bus->stream_list, list) {
		he_stream = stream_to_hdac_ext_stream(s);
		hda_stream = container_of(he_stream, struct sof_intel_hda_stream, he_stream);
...
int hda_dsp_stream_trigger(struct snd_sof_dev *sdev,
			   struct hdac_ext_stream *he_stream, int cmd)
{
	struct hdac_stream *hstream = &he_stream->hstream;
...
int hda_dsp_iccmax_stream_hw_params(struct snd_sof_dev *sdev, struct hdac_ext_stream *he_stream,
				    struct snd_dma_buffer *dmab,
			   ...
			   struct snd_pcm_substream *substream)
{
	...
	hda_dsp_stream_spib_config(sdev, he_stream, HDA_DSP_SPIB_DISABLE, 0);
...
int hda_dsp_stream_init(struct snd_sof_dev *sdev)
{
	...
	for (i = 0; i < num_capture; i++) {
		...
		he_stream = &hda_stream->he_stream;

		he_stream->pphc_addr = sdev->bar[HDA_DSP_PP_BAR] +
			SOF_HDA_PPHC_BASE + SOF_HDA_PPHC_INTERVAL * i;

		he_stream->pplc_addr = sdev->bar[HDA_DSP_PP_BAR] +
			SOF_HDA_PPLC_BASE + SOF_HDA_PPLC_MULTI * num_total +
			SOF_HDA_PPLC_INTERVAL * i;

		/* do we support SPIB */
		if (sdev->bar[HDA_DSP_SPIB_BAR]) {
			he_stream->spib_addr = sdev->bar[HDA_DSP_SPIB_BAR] +
				SOF_HDA_SPIB_BASE + SOF_HDA_SPIB_INTERVAL * i +
				SOF_HDA_SPIB_SPIB;

			he_stream->fifo_addr = sdev->bar[HDA_DSP_SPIB_BAR] +
				SOF_HDA_SPIB_BASE + SOF_HDA_SPIB_INTERVAL * i +
				SOF_HDA_SPIB_MAXFIFO;
		}

		hstream = &he_stream->hstream;
		hstream->bus = bus;
...
void hda_dsp_stream_free(struct snd_sof_dev *sdev)
{
	...
	list_for_each_entry_safe(s, _s, &bus->stream_list, list) {
		...
		list_del(&s->list);
		he_stream = stream_to_hdac_ext_stream(s);
		hda_stream = container_of(he_stream, struct sof_intel_hda_stream,
					  he_stream);
