						 ...
						 int direction)
{
	...
	he_stream = hda_dsp_stream_get(sdev, direction, 0);
	...
	if (direction == SNDRV_PCM_STREAM_CAPTURE) {
		ret = hda_dsp_iccmax_stream_hw_params(sdev, he_stream, dmab, NULL);
		if (ret < 0) {
			dev_err(sdev->dev, "error: iccmax stream prepare failed: %d\n", ret);
			goto error;
		}
	} else {
		ret = hda_dsp_stream_hw_params(sdev, he_stream, dmab, NULL);
		if (ret < 0) {
			dev_err(sdev->dev, "error: hdac prepare failed: %d\n", ret);
			goto error;
		}
		hda_dsp_stream_spib_config(sdev, he_stream, HDA_DSP_SPIB_ENABLE, size);
	}
		      ...
		      struct hdac_ext_stream *he_stream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		return 0;
	default:
		return hda_dsp_stream_trigger(sdev, he_stream, cmd);
	}
}
		      ...
		      struct hdac_ext_stream *he_stream)
{
	...
	if (hstream->direction == SNDRV_PCM_STREAM_PLAYBACK)
		ret = hda_dsp_stream_spib_config(sdev, he_stream, HDA_DSP_SPIB_DISABLE, 0);
	else
...
static int cl_copy_fw(struct snd_sof_dev *sdev, struct hdac_ext_stream *he_stream)
{
	unsigned int reg;
	int ret, status;

	ret = cl_trigger(sdev, he_stream, SNDRV_PCM_TRIGGER_START);
	if (ret < 0) {
	...
	ret = cl_trigger(sdev, he_stream, SNDRV_PCM_TRIGGER_STOP);
	if (ret < 0) {
