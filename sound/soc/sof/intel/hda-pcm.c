				      ...
				      struct snd_pcm_substream *substream)
{
	...
	if (hda && !hda->no_ipc_position) {
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			/*
			 * For capture stream, we need more workaround to fix
			 * the position incorrect issue:
			 *
			 * 1. Wait at least 20us before reading position buffer
			 * after the interrupt generated(IOC), to make sure
			 * position update happens on frame boundary i.e.
			 * 20.833uSec for 48KHz.
			 * 2. Perform a dummy Read to DPIB register to flush DMA
			 * position value.
			 * 3. Read the DMA Position from posbuf. Now the
			 * readback value should be >= period boundary.
			 */
			usleep_range(20, 21);
