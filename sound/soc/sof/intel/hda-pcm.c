			  ...
			  struct sof_ipc_stream_params *ipc_params)
{
	...
	struct hdac_ext_stream *he_stream = stream_to_hdac_ext_stream(hstream);
	...
	ret = hda_dsp_stream_hw_params(sdev, he_stream, dmab, params);
	if (ret < 0) {
	...
	if (hda_disable_rewinds)
		hda_dsp_stream_spib_config(sdev, he_stream, HDA_DSP_SPIB_ENABLE, 0);
	else
