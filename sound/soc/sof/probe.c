...
{
	struct sof_ipc_probe_dma_set_params *msg;
...
static int sof_ipc_probe_get(struct snd_sof_dev *sdev, unsigned int cmd,
		void **params, size_t *bytes)
{
	struct sof_ipc_probe_get_params msg = {0};
	struct sof_ipc_probe_get_params *reply;
	size_t size;
	int ret;
	...
	*bytes = 0;
	...
	size = reply->rhdr.hdr.size -= sizeof(*reply);
	if (!size)
		goto exit;

	*params = kmemdup(&reply->dma[0], size, GFP_KERNEL);
	if (!*params) {
		ret = -ENOMEM;
		goto exit;
	}
...
/**
 * sof_ipc_probe_get_dma - retrieve list of active injection dmas
 * @sdev:	SOF sound device
 * @dma:	Returned list of active dmas
 * @num_dma:	Returned count of active dmas
 *
 * Host sends DMA_GET request to obtain list of injection dmas it
		...
		struct sof_probe_dma **dma, size_t *num_dma)
{
	...
	ret = sof_ipc_probe_get(sdev, SOF_IPC_PROBE_DMA_GET,
			(void **)dma, num_dma);
	if (!ret)
		*num_dma /= sizeof(**dma);
	return ret;
}
EXPORT_SYMBOL(sof_ipc_probe_get_dma);

/**
 * sof_ipc_probe_dma_attach - attach to specified dmas
 * @sdev:	SOF sound device
		...
		struct sof_probe_dma *dma, size_t num_dma)
{
	...
	if (!msg)
		return -ENOMEM;
	msg->hdr.size = sizeof(*msg) + bytes;
	msg->hdr.cmd = SOF_IPC_GLB_PROBE | SOF_IPC_PROBE_DMA_SET;
...
EXPORT_SYMBOL(sof_ipc_probe_dma_attach);

/**
 * sof_ipc_probe_dma_detach - detach from specified dmas
 * @sdev:		SOF sound device
 * @stream_tag:	List of stream tags to detach from
 * @num_stream_tag:	Number of elements in @stream_tag
 *
 * Host sends DMA_DETACH request to free previously attached stream
 * from being occupied for injection. Each detach operation should
 * match equivalent DMA_SET. Detach only when all probes tied to
...
{
	struct sof_ipc_probe_dma_detach_params *msg;
	struct sof_ipc_reply reply;
	...
	if (!msg)
		return -ENOMEM;
	msg->hdr.size = sizeof(*msg) + bytes;
	msg->hdr.cmd = SOF_IPC_GLB_PROBE | SOF_IPC_PROBE_DMA_DETACH;
...
EXPORT_SYMBOL(sof_ipc_probe_dma_detach);

/**
 * sof_ipc_probe_get_points - retrieve list of active probe points
 * @sdev:	SOF sound device
 * @desc:	Returned list of active probes
 * @num_desc:	Returned count of active probes
 *
 * Host sends PROBE_POINT_GET request to obtain list of active probe
		...
		struct sof_probe_point_desc **desc, size_t *num_desc)
{
	...
	ret = sof_ipc_probe_get(sdev, SOF_IPC_PROBE_POINT_GET,
			(void **)desc, num_desc);
	if (!ret)
		*num_desc /= sizeof(**desc);
	return ret;
}
EXPORT_SYMBOL(sof_ipc_probe_get_points);

/**
 * sof_ipc_probe_points_connect - connect specified probes
 * @sdev:	SOF sound device
...
int sof_ipc_probe_points_connect(struct snd_sof_dev *sdev,
		struct sof_probe_point_desc *desc, size_t num_desc)
{
	struct sof_ipc_probe_point_set_params *msg;
	struct sof_ipc_reply reply;
	...
	if (!msg)
		return -ENOMEM;
	msg->hdr.size = sizeof(*msg) + bytes;
	msg->hdr.cmd = SOF_IPC_GLB_PROBE | SOF_IPC_PROBE_POINT_SET;
...
EXPORT_SYMBOL(sof_ipc_probe_points_connect);

/**
 * sof_ipc_probe_points_disconnect - disconnect specified probes
 * @sdev:	SOF sound device
...
EXPORT_SYMBOL(sof_ipc_probe_points_disconnect);
