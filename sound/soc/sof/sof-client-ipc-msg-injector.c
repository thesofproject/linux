					    ...
					    size_t count, loff_t *ppos)
{
	...
	remaining = sizeof(ipc4_msg->header_u64) + ipc4_msg->data_size;
	if (count > remaining)
		count = remaining;
					     ...
					     size_t count, loff_t *ppos)
{
	...
	/* Copy the payload */
	size = simple_write_to_buffer(ipc4_msg->data_ptr, priv->max_msg_size,
				      ppos, buffer, count);
	if (size != count)
		return size > 0 ? -EFAULT : size;
				...
				const struct auxiliary_device_id *id)
{
	...
	priv->rx_buffer = devm_kmalloc(dev, alloc_size, GFP_KERNEL);
