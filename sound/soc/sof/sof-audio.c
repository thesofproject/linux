...
sof_unprepare_widgets_in_path(struct snd_sof_dev *sdev, struct snd_soc_dapm_widget *widget)
{
	...
	if (!widget_ops[widget->id].unprepare || !swidget->complete)
		goto sink_unprepare;

	widget_ops[widget->id].unprepare(swidget);
...
sink_prepare:
	snd_soc_dapm_widget_for_each_sink_path(widget, p) {
		if (!p->walking && p->sink->dobj.private) {
			...
			p->walking = false;
			if (ret < 0) {
				if (!widget_ops[widget->id].unprepare)
					widget_ops[widget->id].unprepare(swidget);
				swidget->complete = false;
				return ret;
			}
			  ...
			  struct snd_pcm_hw_params *runtime_params, int dir)
{
	...
	ret = sof_walk_widgets_in_order(sdev, list, platform_params, runtime_params,
...
widget_free:
	sof_walk_widgets_in_order(sdev, list, platform_params, runtime_params, dir,
				  SOF_WIDGET_FREE);
