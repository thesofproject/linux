...
sof_unprepare_widgets_in_path(struct snd_sof_dev *sdev, struct snd_soc_dapm_widget *widget)
{
	...
	if (!widget_ops[widget->id].ipc_unprepare || !swidget->complete)
		goto sink_unprepare;
			    ...
			    struct snd_pcm_hw_params *source_params, int dir)
{
	...
	if (!widget_ops[widget->id].ipc_prepare || swidget->complete)
		goto sink_prepare;

	/* prepare the source widget */
	ret = widget_ops[widget->id].ipc_prepare(swidget, platform_params, params,
						 source_params, dir);
	if (ret < 0) {
			  ...
			  enum sof_widget_op op)
{
	...
	for_each_dapm_widgets(list, i, widget) {
		...
		{
			struct snd_pcm_hw_params source_params;

			str = "prepare";
			/*
			 * When walking the list of connected widgets, the source_params for each
			 * widget is modified by the source widget in the path. Use a local
			 * copy of the runtime params as the source_params so that the runtime
			 * params does not get overwritten.
			 */
			memcpy(&source_params, params, sizeof(*params));
			  ...
			  struct snd_pcm_hw_params *params, int dir)
{
	 ...
	 */
	ret = sof_walk_widgets_in_order(sdev, list, platform_params, params,
...
widget_free:
	sof_walk_widgets_in_order(sdev, list, platform_params, params, dir,
				  SOF_WIDGET_FREE);
