				      ...
				      struct snd_pcm_substream *substream, int state, int cmd)
{
	...
	/*
	 * Every pipeline of a chained DMA stream should have use_chain_dma attribute, so
	 * this loop is only necessary for checking the correctness of the topology. The loop
	 * together with the check in sof_ipc4_chain_dma_trigger() completes the check.
	 */
	for (i = pipeline_list->count - 1; i >= 0; i--) {
		...
		/*
		 * If any pipeline in the list has use_chain_dma attribute we proceed to
