			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	dai = mod_map[asoc_cpu_dai(rtd, 0)->id].dai_id;
	...
	if (pcm_data->allocated) {
		...
		pcm_data->stream = sst_hsw_stream_new(hsw, asoc_cpu_dai(rtd, 0)->id,
	...
	ret = create_adsp_page_table(substream, pdata, rtd, runtime->dma_area,
		runtime->dma_bytes, asoc_cpu_dai(rtd, 0)->id);
	if (ret < 0)
		return ret;
	...
	ret = sst_hsw_stream_buffer(hsw, pcm_data->stream,
		pdata->dmab[asoc_cpu_dai(rtd, 0)->id][substream->stream].addr,
		       ...
		       struct snd_soc_pcm_runtime *rtd)
{
	...
	if (pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream)
		priv_data->pcm[asoc_cpu_dai(rtd, 0)->id][SNDRV_PCM_STREAM_PLAYBACK].hsw_pcm = pcm;
	if (pcm->streams[SNDRV_PCM_STREAM_CAPTURE].substream)
		priv_data->pcm[asoc_cpu_dai(rtd, 0)->id][SNDRV_PCM_STREAM_CAPTURE].hsw_pcm = pcm;
