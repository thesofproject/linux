...
{
	struct snd_soc_component *codec = asoc_codec_dai(runtime, 0)->component;
	...
	ret = snd_soc_dai_set_sysclk(asoc_codec_dai(runtime, 0), 0, 19200000,
				     SND_SOC_CLOCK_IN);
	if (ret < 0) {
			    ...
			    struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_tdm_slot(asoc_cpu_dai(rtd, 0), 0x3, 0x3, 2, bits);
	if (ret < 0) {
