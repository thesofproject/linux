...
#define NEXT_ID(BE, CPU, LINK)	{ (BE)++; (CPU)++; (LINK)++; }
 ...
 *
 * one slave on one sdw link without aggregated = 0
 * one sdw BE DAI <---> one-cpu DAI <---> one-codec DAI
 *
 * two or more slaves on one sdw link without aggregated = 0:
			      ...
			      int *cpu_id, bool *group_generated)
{
	...
	int codec_idx = 0, stream = 0;
	int i, j = 0, k, codec_index;
	int codec_num;
	int ret;
	...
	for (link_next = link, i = 0; link_next && link_next->num_adr &&
	...
	for_each_pcm_streams(stream) {
		 ...
		 * PIN ID with bias of 2 according to sdw dai driver.
		 */
