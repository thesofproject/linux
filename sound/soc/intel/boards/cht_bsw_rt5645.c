...
static int cht_codec_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	struct snd_soc_component *component = asoc_codec_dai(runtime, 0)->component;
	int jack_type;
	int ret;
			    ...
			    struct snd_pcm_hw_params *params)
{
	...
	if ((cht_rt5645_quirk & CHT_RT5645_SSP0_AIF1) ||
		 ...
		 */
		ret = snd_soc_dai_set_fmt(asoc_cpu_dai(rtd, 0),
					SND_SOC_DAIFMT_I2S     |
		...
		ret = snd_soc_dai_set_fmt(asoc_codec_dai(rtd, 0),
					SND_SOC_DAIFMT_I2S     |
					SND_SOC_DAIFMT_NB_NF   |
					SND_SOC_DAIFMT_CBS_CFS
			);
		...
		ret = snd_soc_dai_set_tdm_slot(asoc_cpu_dai(rtd, 0), 0x3, 0x3, 2, 16);
		if (ret < 0) {
		...
		ret = snd_soc_dai_set_tdm_slot(asoc_codec_dai(rtd, 0), 0xF, 0xF, 4, 24);
		if (ret < 0) {
