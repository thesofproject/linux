				...
				struct snd_pcm_substream *substream)
{
	...
	return snd_soc_dai_get_dma_data(asoc_cpu_dai(soc_runtime, 0), substream);
}
...
static void disable_intr(struct snd_pcm_substream *substream)
{
	...
	dev_dbg(asoc_cpu_dai(rtd, 0)->dev, "%s on port %d\n", __func__, aio->portnum);
			   ...
			   struct snd_pcm_substream *substream)
{
	...
	dev_dbg(asoc_cpu_dai(rtd, 0)->dev, "%s port %d\n", __func__, aio->portnum);
			    ...
			    struct snd_pcm_substream *substream)
{
	...
	if (!aio->play_stream && !aio->capture_stream)
		dev_dbg(asoc_cpu_dai(rtd, 0)->dev, "freed  port %d\n", aio->portnum);
			      ...
			      struct snd_pcm_substream *substream)
{
	...
	dev_dbg(asoc_cpu_dai(rtd, 0)->dev, "%s (buf_size %lu) (period_size %lu)\n",
...
static int cygnus_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	dev_dbg(asoc_cpu_dai(rtd, 0)->dev, "%s: size 0x%zx @ %pK\n",
				__func__, size, buf->area);

	if (!buf->area) {
		dev_err(asoc_cpu_dai(rtd, 0)->dev, "%s: dma_alloc failed\n", __func__);
		return -ENOMEM;
	}
	buf->bytes = size;
