...
static int get_acp63_cpu_pin_id(u32 sdw_link_id, int be_id, int *cpu_pin_id, struct device *dev)
{
	...
	case AMD_SDW0:
		switch (be_id) {
		case ASOC_SDW_JACK_OUT_DAI_ID:
			*cpu_pin_id = SW0_AUDIO0_TX;
			break;
		case ASOC_SDW_JACK_IN_DAI_ID:
			*cpu_pin_id = SW0_AUDIO0_RX;
			break;
		case ASOC_SDW_AMP_OUT_DAI_ID:
			*cpu_pin_id = SW0_AUDIO1_TX;
			break;
		case ASOC_SDW_AMP_IN_DAI_ID:
			*cpu_pin_id = SW0_AUDIO1_RX;
			break;
		case ASOC_SDW_DMIC_DAI_ID:
...
static int mc_probe(struct platform_device *pdev)
{
	...
	int codec_info_list_size;
	int amp_num = 0, i;
	int ret;

	codec_info_list_size = asoc_sdw_get_codec_info_list_size();
