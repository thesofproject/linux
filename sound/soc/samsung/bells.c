				...
				enum snd_soc_bias_level level)
{
	...
	codec_dai = asoc_codec_dai(rtd, 0);
...
static int bells_late_probe(struct snd_soc_card *card)
{
	...
	wm0010 = asoc_codec_dai(rtd, 0)->component;

	rtd = snd_soc_get_pcm_runtime(card, &card->dai_link[DAI_DSP_CODEC]);
	component = asoc_codec_dai(rtd, 0)->component;
	aif1_dai = asoc_codec_dai(rtd, 0);
	...
	aif3_dai = asoc_cpu_dai(rtd, 0);
	wm9081_dai = asoc_codec_dai(rtd, 0);
