			 ...
			 struct snd_pcm_hw_params *params)
{
	...
	dma_params = snd_soc_dai_get_dma_data(asoc_cpu_dai(rtd, 0), substream);
	if (!dma_params)
		return 0;
	...
	snd_dmaengine_pcm_set_config_from_dai_data(substream,
			snd_soc_dai_get_dma_data(asoc_cpu_dai(rtd, 0), substream),
			&config);
...
int pxa2xx_pcm_open(struct snd_pcm_substream *substream)
{
	...
	return snd_dmaengine_pcm_open(
		substream, dma_request_slave_channel(asoc_cpu_dai(rtd, 0)->dev,
